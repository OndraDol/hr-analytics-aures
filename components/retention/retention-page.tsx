      ...
      <section className="grid gap-6 xl:grid-cols-[1.35fr_0.65fr]">
        <div className="rounded-lg border border-zinc-200 bg-white p-6 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="max-w-3xl">
              <p className="text-sm font-semibold uppercase tracking-[0.2em] text-blue-700">Retention analytics</p>
              <h1 className="mt-3 text-4xl font-semibold tracking-normal text-zinc-950 md:text-5xl">
                Fluktuace jako řízené riziko, ne jen historické číslo
              </h1>
              <p className="mt-4 max-w-2xl text-base leading-7 text-zinc-600">
                Referenční sekce ukazuje, jak dashboard vyhodnocuje status metrik, hledá hlavní příčiny změny a převádí výsledek do konkrétní akce pro HR a manažery.
              </p>
            </div>
            <Link
              href="#drivers"
        ...
        <div id="ai" className="rounded-lg border border-violet-200 bg-violet-50 p-6 shadow-sm">
          ...
          <p className="mt-5 text-sm leading-6 text-violet-900">
            AI text je záměrně oddělený od deterministického výpočtu. Čísla a statusy drží analytický engine, AI pouze pomáhá formulovat hypotézu.
          </p>
        </div>
      </section>
            ...
            {riskSegments.map((segment) => (
              <div key={segment.label} className="rounded-md border border-zinc-200 bg-zinc-50 p-3">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="rounded-full bg-white px-2 py-1 text-xs text-zinc-600">{segment.severity}</span>
                </div>
