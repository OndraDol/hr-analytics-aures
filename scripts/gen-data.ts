 ...
 *   reálná data (staffplan + workforce events + recruitment) → kostra
...
function main(): void {
  ...
  console.log(
    `   real data: ${sp.positions.length} positions, ${wf.events.length} wf events, ${rec.rows.length} recruitment rows`,
  );
  ...
  const matchPosition = (orgUnitName: string): Position => {
    ...
    const byDept = positions.find((p) => {
      const firstWord = p.title.toLowerCase().split(' ')[0] ?? '__none__';
      return firstWord.length > 2 && name.includes(firstWord);
    });
  ...
  for (const [oscpv, info] of wf.employees) {
    const rngG = seedrandom(`${SEED}:g:${oscpv}`);
    const rngTr = seedrandom(`${SEED}:tr:${oscpv}`);
    const rngBd = seedrandom(`${SEED}:bd:${oscpv}`);
    const g: Gender = rngG() < 0.75 ? 'male' : 'female';
    const names = pseudo.pseudonymize(oscpv, g);
  ...
  for (const emp of employees) {
    payroll.push(
      ...generatePayrollForEmployee(emp, { fromMonth: START_MONTH, toMonth: END_MONTH, seed: SEED }),
    );
    ...
    for (const cycle of ['2024', '2025']) {
      ...
      const termYear = emp.terminationDate ? new Date(emp.terminationDate).getFullYear() : Infinity;
      const cycleYear = Number(cycle);
      if (hireYear <= cycleYear && termYear >= cycleYear) {
        perf.push(generatePerformanceReview(emp, { cycle, seed: SEED }));
      }
    }
  }
  ...
  for (const e of activeEmployees) {
    divisionHc.set(e.divisionId, (divisionHc.get(e.divisionId) ?? 0) + 1);
  }
  ...
  for (const p of positions.filter((x) => x.criticalFlag)) {
  ...
  console.log(`   employees: ${employees.length}, payroll: ${payroll.length}, absence: ${absence.length}`);
  console.log(`   perf: ${perf.length}, enps: ${enps.length}, training: ${training.length}`);
  console.log(`   accidents: ${accidents.length}, succession: ${successionPlans.length}`);
  console.log(`   📄 ${OUT}`);
}
