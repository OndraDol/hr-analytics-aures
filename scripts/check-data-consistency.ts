...
for (const a of ABSENCE) {
  ...
  if (!e) {
    errors.push(`Absence for unknown employee ${a.employeeId}`);
    continue;
  }
  if (a.dateFrom < e.hireDate) errors.push(`Absence ${a.employeeId} before hire date`);
  if (e.terminationDate && a.dateFrom > e.terminationDate) {
    errors.push(`Absence ${a.employeeId} after termination`);
  }
}
...
for (const p of PERFORMANCE) {
  ...
  if (!e) {
    errors.push(`Performance for unknown employee ${p.employeeId}`);
    continue;
  }
  const year = Number(p.cycle);
  if (new Date(e.hireDate).getFullYear() > year) {
    errors.push(`Performance ${p.employeeId} cycle before hire`);
  }
